...
input_filename = '../meshes/mesh-cube-40.txt';
% Load nodes form mesh in format X Y Z
fileID = fopen(input_filename, 'r');
all_points = fscanf(fileID, '%f', [3, Inf])'; 
fclose(fileID);
...
scatter3(inside_points(:, 1), inside_points(:, 2), inside_points(:, 3), 'g'); % Green for inside
scatter3(outside_points(:, 1), outside_points(:, 2), outside_points(:, 3), 'r'); % Red for outside
